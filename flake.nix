{
  ...
  outputs =
    ...
    }:
    {
      ...
      darwinConfigurations."Yanniks-MacBook-Pro-M4" = darwin.lib.darwinSystem {
        system = "aarch64-darwin";
        modules = [
          (import ./common { inherit nixpkgs; })
          ./machines/mbp-2024
      ...
      homeConfigurations."yannik@mbp-2024" =
        let
          username = "yannik";
          system = "aarch64-darwin";
        in
      ...
      packages = nixpkgs.lib.genAttrs [ "x86_64-linux" "x86_64-darwin" "aarch64-darwin" ] (
        arch:
        let
          homeManagerPackages = home-manager.packages."${arch}";
          nixDarwinPackages = darwin.packages.${arch};
        in
        ...
        // nixpkgs.lib.optionalAttrs (arch == "aarch64-darwin" || arch == "x86_64-darwin") {
          inherit (nixDarwinPackages) darwin-rebuild;
        }

      );

      devShells = nixpkgs.lib.genAttrs [ "x86_64-linux" "x86_64-darwin" "aarch64-darwin" ] (
        arch:
        let
          nixpkgsPackages = nixpkgs.legacyPackages."${arch}";
          homeManagerPackages = home-manager.packages."${arch}";
          nixDarwinPackages = darwin.packages.${arch};
        in

        {
          default = nixpkgsPackages.mkShell {
            packages =
              [
                nixpkgsPackages.nixd
                nixpkgsPackages.nixfmt-rfc-style
                homeManagerPackages.home-manager
              ]
              ++ nixpkgs.lib.optional (arch == "aarch64-darwin" || arch == "x86_64-darwin") [
                nixDarwinPackages.darwin-rebuild
              ];
          };
        }
      );
